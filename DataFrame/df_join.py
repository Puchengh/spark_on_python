...
sql_df1 = ss.sql('select name,gender from stu')

sql_df2 = ss.sql('select gender,sum(age) from stu group by gender')

#默认是内关联
sql_df3 = sql_df1.join(sql_df1, 'gender', 'left')
sql_df4 = sql_df1.join(sql_df1, 'gender', 'right')
